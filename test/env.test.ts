...
import {loadEnv, env} from '../src/env.js';
...
describe('ENV', () => {
	...
	test('should be able to load multiple environments not local', () => {
		...
		loadEnv({path: './test/fixtures/env-file-load', environment: 'development', local: false});
	...
	test('should be able to load multiple environments not local', () => {
		delete process.env.KEY_ID;
		expect(process.env.KEY_ID).toBeUndefined();
		env({path: './test/fixtures/env-file-load', environment: 'development', local: false});
		expect(process.env.KEY_ID).toBe('development_key_id');
	});
